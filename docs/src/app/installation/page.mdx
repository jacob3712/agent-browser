...
This copies `agent-browser` into `tools/agent-browser` inside the target project, reuses the current native binary when available, and then tries to install Chrome. See `README-USER.md` in the repository root for the simple end-user flow.
